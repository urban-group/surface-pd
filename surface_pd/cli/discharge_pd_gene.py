...
def add_composition2df(
    ...
    if data2 is None:
        ...
        df2_li = find_max_min_composition(
            df2, column_name=lithium_like_species
        )
        df2_o = find_max_min_composition(df2, column_name=oxygen_like_species)

    df1_li = find_max_min_composition(df1, column_name=lithium_like_species)
    df1_o = find_max_min_composition(df1, column_name=oxygen_like_species)
...
def main(argv: list[str] | None = None) -> None:
    """Run the discharge phase diagram data generation CLI."""
    parser = argparse.ArgumentParser(
        description=(
            "Generate discharge phase diagram data from charge data.\n\n"
            "This tool processes charge surface phase diagram data to "
            "create corresponding discharge data by modifying energies "
            "based on oxygen composition.\n\n"
            "Example usage:\n"
            "  %(prog)s charge-data.dat -x 0.8 --save\n"
            "  %(prog)s file1.dat file2.dat -L Li -O O -x 0.75 -s"
        ),
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path(s) to charge surface phase diagram data file(s). "
            "Same format as surface-pd-plot input."
        ),
    )
